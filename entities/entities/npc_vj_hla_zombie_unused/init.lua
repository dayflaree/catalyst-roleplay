...
ENT.Model = {"models/creatures/zombies/zombie_v2.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
...
--ENT.HitGroupFlinching_Values = {{HitGroup = {HITGROUP_HEAD},{HitGroup = {HITGROUP_CHEST}, Animation = {ACT_FLINCH_CHEST}},{HitGroup = {HITGROUP_STOMACH}, Animation = {ACT_FLINCH_STOMACH}}, Animation = {ACT_FLINCH_HEAD}}, {HitGroup = {HITGROUP_LEFTARM}, Animation = {ACT_FLINCH_LEFTARM}}, {HitGroup = {HITGROUP_RIGHTARM}, Animation = {ACT_FLINCH_RIGHTARM}}}
...
function ENT:CustomOnTakeDamage_BeforeDamage(dmginfo,hitgroup)
	if hitgroup == 11 then
--		self:VJ_ACT_PLAYACTIVITY("vjges_ACT_GESTURE_FLINCH_HEAD",true,1.075,false)
		self:AddGesture(ACT_GESTURE_FLINCH_HEAD)
		if dmginfo:IsBulletDamage() then
		--	dmginfo:AddDamage(1.75)
			dmginfo:ScaleDamage(1.75)
		--	dmginfo:SetDamage(1.75)
		end
...
function ENT:CustomOnInitialKilled(dmginfo, hitgroup)
	...
	if IsValid(dmginfo:GetInflictor()) and dmginfo:GetInflictor():GetModel() == "models/props_junk/sawblade001a.mdl" then
		self.HasDeathRagdoll = false
		self.HasDeathAnimation = false
		local legs = ents.Create("prop_ragdoll")
		legs:SetModel("models/creatures/zombies/zombie_v2_legs.mdl")
		legs:SetPos(self:GetPos())
		legs:SetAngles(self:GetAngles())
		legs:Spawn()
		legs:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
		local torso = ents.Create("npc_vj_hla_zombietorsoclassic")
		torso:SetPos(self:GetPos()+Vector(0,0,50))
	...
	if self.TORSO == false then
		if hitgroup == 11 and GetConVarNumber("vj_hla_enable_hard_difficulty") == 0 or self.inflatonrate == true then
