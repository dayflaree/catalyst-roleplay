...
ix.lang.AddTable("english", {
    ...
    optNpcESP = "Show NPC ESP",
    optdNPCESP = "Shows the names and locations of each NPC in the server."
})
...
if ( CLIENT ) then
    ...
    ix.option.Add("npcESP", ix.type.bool, true, {
        category = "observer",
        hidden = function()
            return !CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Observer", nil)
        end
    ...
    function PLUGIN:HUDPaint()
        ...
        if (ix.option.Get("observerESP", true) and client:GetMoveType() == MOVETYPE_NOCLIP and
            ...
            for _, v in player.Iterator() do
                ...
                if (v == client or !v:GetCharacter() or client:GetAimVector():Dot((v:WorldSpaceCenter() - client:WorldSpaceCenter()):GetNormal()) < 0.65) then
                    continue
                end

                local screenPosition = v:WorldSpaceCenter():ToScreen()
                local aimPosition = (v:WorldSpaceCenter() + v:GetAimVector() * aimLength):ToScreen()
                ...
                local distance = client:WorldSpaceCenter():Distance(v:WorldSpaceCenter())
        ...
        if (ix.option.Get("itemESP", true) and client:GetMoveType() == MOVETYPE_NOCLIP and
            !client:InVehicle() and CAMI.PlayerHasAccess(client, "Helix - Observer", nil)) then
            local scrW, scrH = ScrW(), ScrH()
            ...
            for _, v in ents.Iterator() do
                ...
                ix.util.DrawText(text, x, y - size, ColorAlpha(ix.config.Get("color"), alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
            end
        end

        if (ix.option.Get("npcESP", true) and client:GetMoveType() == MOVETYPE_NOCLIP and
            !client:InVehicle() and CAMI.PlayerHasAccess(client, "Helix - Observer", nil)) then
            local scrW, scrH = ScrW(), ScrH()
            ...
            for _, v in ents.Iterator() do
                ...
                if !string.find(v:GetClass(), "npc_") or !v:IsNPC() then
                    continue
                end
                ...
                local text = v.PrintName or v:GetClass()
